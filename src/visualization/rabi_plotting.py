...
def load_history() -> Optional[Dict]:
    possible_paths = [
        project_root / 'results' / 'checkpoints' / 'rabi' / 'training_history.npy',
        project_root / 'checkpoints' / 'rabi' / 'training_history.npy',
        project_root / 'results' / 'training_history.npy'
    ]
...
def plot_fig1c_alpha():
    ...
    alpha_data = None
    if history:
        # Check standard keys for alpha evolution
...
def plot_fig1d_discovery():
    ...
    if not ckpt_path.exists():
        raise FileNotFoundError(f"CRITICAL: Model checkpoint not found at {ckpt_path}. Cannot generate discovery plot.")
...
def plot_fig2_benchmark():
    ...
    mse_block = loaded.get("benchmark_mse") if isinstance(loaded, dict) else None
    if isinstance(mse_block, dict):
        loaded_mse = mse_block
    ...
    data = {
        "RF": loaded_mse.get("RF"),
        "XGB": loaded_mse.get("XGB"),
        "PINN (Inc)": loaded_mse.get("PINN_Incomplete") or loaded_mse.get("PINN (Inc)"),
        "PINN (Known)": loaded_mse.get("PINN_Known") or loaded_mse.get("PINN (Known)"),
        "GAN": loaded_mse.get("GAN"),
        "ParaQNN": loaded_mse.get("ParaQNN")
    }
    ...
    for m in models:
        val = data.get(m)
        if val is None:
            print(f"Warning: Missing metric for {m}, skipping plot generation for this bar.")
