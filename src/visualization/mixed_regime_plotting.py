...
def load_history() -> Optional[Dict]:
    possible_paths = [
        project_root / 'results' / 'checkpoints' / 'mixed' / 'training_history.npy',
...
def plot_fig5c_alpha():
    ...
    if history:
        ...
        for k in ("alpha_first", "alpha_history", "alpha_values", "alpha"):
            if k in history:
...
def plot_fig5d_discovery():
    ...
    try:
        # Model config must match Mixed Regime settings (128 neurons, 3 layers)
