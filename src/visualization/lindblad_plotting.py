...
def load_history() -> Optional[Dict]:
    possible_paths = [
        project_root / 'results' / 'checkpoints' / 'lindblad' / 'training_history.npy',
...
def plot_fig3a_simulation():
    ...
    save_path = get_save_dir() / 'Fig3a_Lindblad_Simulation.pdf'
...
def plot_fig3c_alpha():
    ...
    if alpha_data is not None:
        ax.plot(alpha_data, color=COLORS['alpha'], linewidth=2.0, label=r'$\alpha$')
        ax.set_xlabel("Epochs")
...
def plot_fig4_benchmark():
    ...
    try:
        with open(metrics_path, 'r') as f:
            loaded = json.load(f)
            data['RF'] = loaded.get('RF')
            data['XGB'] = loaded.get('XGB')
            data['PINN (Inc)'] = loaded.get('PINN_Incomplete')
            data['PINN (Known)'] = loaded.get('PINN_Known')
            data['GAN'] = loaded.get('GAN')
            data['ParaQNN'] = loaded.get('ParaQNN')
    except Exception as e:
    ...
    for m in models:
        val = data.get(m)
        if val is None:
            mse_values.append(0.0) # Or handle appropriately
        else:
